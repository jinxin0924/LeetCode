...
def fourSum(nums, target):  #O(n3)
    ...
    """
        ...
        for first in range(l-3):
            if nums[first]*4>target: #add constraints can make comparison numbers reduced by 90%
                break
            for second in range(first+1,l-2):
                if nums[second]*3>target-nums[first]: #
                    break
                third=second+1
                fourth=l-1
                while third<fourth:
                    list1=[nums[first],nums[second],nums[third],nums[fourth]]
                    current=sum(list1)
                    if current==target:
